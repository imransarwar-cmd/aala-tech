...
# NOTE: The original module overrode odoo.addons.web.controllers.main.Home
# to strip the X-Frame-Options header from /web/login. That import path was
# removed from Odoo (Home moved to odoo.addons.web.controllers.home back in
# v15), and the override has nothing to do with e-invoicing/ZATCA - it
# disables clickjacking protection on the login page for every user of the
# database. It has been dropped in this v19 port. If you genuinely need to
# embed the Odoo login page in an iframe, that should be a deliberate,
# separate decision (e.g. via reverse-proxy headers), not silently bundled
# into a tax-compliance module.
