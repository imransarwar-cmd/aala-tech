...

{
    ...
    "depends": [
        'base', 'web', 'account',
        # NOTE: report/account_move.xml renders doc.l10n_sa_qr_code_str, which
        # is a field from Odoo's own native Saudi Arabia localization, not a
        # field this module defines. If 'l10n_sa' is not installed/depended
        # on, that field will not exist on account.move and the QR code image
        # in the PDF will silently fail to render (or error, depending on
        # context). Add 'l10n_sa' here once you've confirmed how you want to
        # handle the overlap with native ZATCA support (see chat notes).
    ],
